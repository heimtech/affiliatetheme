...
if ($_GET['secret'] != md5(trim($tradeDoublerApiUrl))) {
    echo 'Keine Berechtigung';
    die();
}

$posttype = 'produkt';
$customfieldPid = 'tradedoubler_product_id';
...
$args = array(
    'meta_key' => 'tradedoubler_product_id',
    'post_type' => $posttype,
    'post_status' => 'any',
    'posts_per_page' => - 1
);
...
if ($postedProducts) {
    ...
    foreach ($postedProducts as $postedProduct) {
        ...
        if ($productId) {
            ...
            // produkt von tradedoubler abholen
            if ($product = $affiliatePartner->getApSingleProduct($productId, 'tradedoubler')) {
                ...
                echo '
                        <tr class="error">
			                ...
			                <td colspan="3">
			                     FEHLER: Request failed. Tradedoubler-Server überlastet oder Produkt kann nicht ermittelt werden.
			                     Wenn dieses Problem weiterhin besteht,
			                     wenden Sie sich bitte an den Support von Tradedoubler.
			                </td>
			            </tr>';
            }
        } else {
