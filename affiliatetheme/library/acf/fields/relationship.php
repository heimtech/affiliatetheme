...
if (! class_exists('acf_field_relationship')) :
    ...
    class acf_field_relationship extends acf_field
    {
        ...
        function __construct()
        {
            ...
            // extra
            add_action('wp_ajax_acf/fields/relationship/query', array(
                $this,
                'ajax_query'
            ));
            add_action('wp_ajax_nopriv_acf/fields/relationship/query', array(
                $this,
                'ajax_query'
            ));
            ...
            if (! empty($field['elements'])) {
                ...
                if (in_array('featured_image', $field['elements'])) {
                    ...
                    if ($post->post_type == 'attachment') {
                        ...
                        $image = wp_get_attachment_image($post->ID, array(
                            17,
                            17
                        ));
                    } else {
                        
                        $image = get_the_custom_post_thumbnail($object_id, 'img_by_url_image_w17', null, array(
                            17,
                            17
                        ));
                    }
...
if (! empty($field['value'])) :
                ...
                if (! empty($posts)) :
                    ...
                    foreach (array_keys($posts) as $i) :
                        ...
                        ?><li><input type="hidden" name="<?php echo $field['name']; ?>[]"
					value="<?php echo $post->ID; ?>" /> <span
					data-id="<?php echo $post->ID; ?>" class="acf-rel-item">
