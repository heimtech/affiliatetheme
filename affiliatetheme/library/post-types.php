...
function produkt_register()
{
    ...
    if (trim($use_custom_permalinks) === '1') {
        $args['rewrite'] = array(
            'slug' => $brand_singular_slug . '/%produkt_marken%'
        );
    } else {
        $args['rewrite'] = array(
            'slug' => '/produkt/%produkt_marken%'
        );
    }
...
add_post_type_support('produkt', array(
    'comments'
));
    ...
    function slideshow_custom_columns($column)
    {
        ...
        switch ($column) {
            case "slideshow_thumbnail":
                the_custom_post_thumbnail($post, 'img_by_url_image_w400', null, array(
                    400,
                    300
                ));
...
<tr>
	...
	<td>
                    <?php
            $customTaxLinkList = array();
            foreach ($cus_tax as $my_tax) {
                $customTaxLinkList[] = '<a href="' . get_bloginfo('url') . '/' . $custom_taxs_objs[$key]->taxonomy_slug . '/' . $my_tax->slug . '/"><span>' . $my_tax->name . '</span></a>';
            }
            
            if (count($customTaxLinkList) > 0) {
                echo implode(', ', $customTaxLinkList);
            }
            ?>
                </td>
</tr>
