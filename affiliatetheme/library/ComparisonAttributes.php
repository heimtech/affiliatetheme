...
class ComparisonAttributes
{
    ...
    function __construct()
    {
        ...
        $this->affiliseoOptions = get_option(AFFILISEO_THEMEOPTIONS);
    }
    ...
    public function getProductPriceValue($postId)
    {
        $ret = '';
    ...
    public function getProductUvpPriceValue($postId)
    {
        ...
        $ret = $this->getProductPriceValue($postId);
        ...
        if (! empty($this->affiliseoOptions['show_uvp'])) {
            ...
            if (! empty($uvp) && $showUvp == '1' && strToFloat($uvp) > strToFloat(get_field('preis', $postId))) {
                
                $currencyBefore = ($pos_currency == 'before') ? $currency_string . ' ' : '';
                $currencyAfter = ($pos_currency == 'after') ? ' ' . $currency_string : '';
                
                $ret .= '<span class="uvp-line-through">';
    ...
    {
        $pdpButtonLabel = get_theme_mod('affiliseo_buttons_detail_label', 'Produktbeschreibung &rsaquo;');
    ...
    public function getProductImageValue($postId)
    {
        ...
        return '<a href="' . $pdp . '" target="_blank">' . $ret . '</a>';
    }
...
SQL;
    ...
    public function writeHighlightedColumnHeadline($productId)
    {
        $ret = '';
        ...
        if (intval($highlightedColumn) == 1) {
            ...
            $ret .= '<td class="comparison_highlighted_column" style=" background-color:' . $bgColor . '; color:' . $highlightedColumnColor . '; ' . $this->writeHighlightedColumnStyle($productId) . ' ">' . $highlightedColumnHeadline . '</td>';
        }
